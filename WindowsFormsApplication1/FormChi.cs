...
{
    public partial class FormChi : Form
    {
        public int cantidad;
        int numeroclases = 0;
        ...
        private void FormChi_Load(object sender, EventArgs e)
        {
            LLenarGrid();
            gridVChi2.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.None; //se ajustan las

            for (int i = 0; i < gridVChi2.Columns.Count; i++)
            {
                //int colw = gridVChi2.Columns[i].Width;
                //gridVChi2.Columns[i].AutoSizeMode = DataGridViewAutoSizeColumnsMode.None; //se ajustan las
                gridVChi2.Columns[i].Width = 70;
            }
        }
        ...
        {
            string archivo = "D:\\05_Tabla_Chicuadrado.xlsx";
            string hoja = "Sheet1";
            ...
            else
            {
                try
                {
                    ...
                    gridVChi2.DataSource = dataSet.Tables[0]; //le asignamos al DataGridView el contenido del dataSet
                    conexion.Close();//cerramos la conexion
                    gridVChi2.AllowUserToAddRows = false;       //eliminamos la ultima fila del datagridview que se autoagrega
                }
                catch (Exception ex)
                {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            if (GralibtxtBox.Text == "" && clasetxtBox.Text == "")
            {
            ...
            if (clasetxtBox.Text != "")
            {
                numeroclases = Convert.ToInt32(clasetxtBox.Text);
                division = numeroclases;
                p = 1 / division;

            }
            if (GralibtxtBox.Text != "")
            {

                numeroclases = Convert.ToInt32(GralibtxtBox.Text) + 1;
                division = numeroclases;
                p = 1 / division;
            ...
            for (cont = 0; cont < numeroclases; cont++)
            {
                ...
                string columna0, columna1 = "", columna2, columna3, columna4;
                ...
                ei = cantidad * p;
                columna3 = ei.ToString();
                chi2 = Math.Pow((sumar - ei), 2) / ei;
                columna4 = chi2.ToString();

                gridChiDatos.Rows.Add(Convert.ToInt32(columna0) + 1, columna1, columna2, columna3, columna4);
        ...
        public void pruebauniformidad()
        {
            ...
            for (cont = 0; cont < numeroclases; cont++)
            {
                ...
                valor = gridChiDatos[4, cont].Value.ToString();
                sumas = sumas + Convert.ToDouble(valor);
            }

            for (cont1 = 1; cont1 < gridVChi2.Rows.Count; cont1++)
            {
                if (numeroclases == Convert.ToInt32(gridVChi2[0, cont1].Value.ToString()))
                {
                    vp = cont1;
                }
                if (cont1 == gridVChi2.Rows.Count && (1 - (Convert.ToDouble(txtNivelConfianza.Text) / 100)) != Convert.ToInt32(gridVChi2[cont1, 0].Value.ToString()))
                {
                    MessageBox.Show("No existe esa cantidad de numeros para la tabla");
                }
            }

            for (cont2 = 1; cont2 < gridVChi2.Columns.Count; cont2++)
            {
                decimal confianza = (1 - (Convert.ToDecimal(txtNivelConfianza.Text) / 100));
                if (confianza == Convert.ToDecimal(gridVChi2[cont2, 0].Value.ToString()))
                {
                    nivelconfianza = cont2;
                }
                if (cont2 == gridVChi2.Columns.Count && confianza != Convert.ToDecimal(gridVChi2[cont2, 0].Value.ToString()) && nivelconfianza == 0)
                {
            ...
            gridVChi2.CurrentCell = gridVChi2[nivelconfianza, vp];
            gridVChi2[nivelconfianza, vp].Style.BackColor = Color.Yellow;
            if (sumas <= Convert.ToDouble(gridVChi2[nivelconfianza, vp].Value.ToString()))
            {
        ...
        public void LlenarArribaAbajo()
        {
            ...
            double cantidadh = 0;
            int valor = 0;
            for (cont = 0; cont < gridVDatos.RowCount - 1; cont++)
            {
            ...
            if (Z < Convert.ToDouble(gridVChi2[vp, (nivelconfianza / 2)].Value.ToString()))
            {
        ...
        private void GralibtxtBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (GralibtxtBox == sender)
            {
